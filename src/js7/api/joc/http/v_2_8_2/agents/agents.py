from ......model.private.api.endpoint import EndpointCall, EndpointDefinition
from ......model.private.http.joc.joc_v_2_8_2 import ReadAgentsV, AgentsV


def agents(call: EndpointCall) -> AgentsV:    
    if not call.payload:
        raise ValueError("'payload' is required in function call.")
    
    if not call.access_token:
        raise ValueError("'access_token' ist required in function call")
    
    if not isinstance(call.payload, ReadAgentsV):
        raise ValueError("'payload' is not an instance of model.http.joc_v_2_8_2.ReadAgentsV.")
    
    body = call.payload.model_dump_json(by_alias=True, exclude_none=True)
    
    with call.http_service as http:
        resp = http.post(
            path="/joc/api/agents",
            body=body,
            headers={
                "X-Access-Token": call.access_token, 
                "Accept": "application/json"
            }
        )
        
        return AgentsV.model_validate_json(resp)
    
ENDPOINT_DEFINITION = EndpointDefinition(
    id="agents",
    function=agents,
    version=("2.6.5", "2.8.3"),
    payload_model=ReadAgentsV,
    response_model=AgentsV,
    options=None
)
